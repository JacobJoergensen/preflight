...
func TestWorkerCount(t *testing.T) {
	tests := []struct {
		...
		{name: "zero jobs returns 1", jobCount: 0, wantMin: 1, wantMax: 1},
		{name: "negative jobs returns 1", jobCount: -5, wantMin: 1, wantMax: 1},
		{name: "single job returns 1", jobCount: 1, wantMin: 1, wantMax: 1},
		{name: "many jobs capped at 8", jobCount: 100, wantMin: 1, wantMax: 8},
		{name: "few jobs limited by job count", jobCount: 3, wantMin: 1, wantMax: 3},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := workerCount(tt.jobCount)
...
func TestCollect(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			results := Collect(context.Background(), tt.jobs, work)
...
func TestCollectReturnsWhenContextCanceled(t *testing.T) {
